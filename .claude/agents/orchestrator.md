...
- Cannot write product code or tests — that is the builder's work, and completeness is
  the checker's word.
...
and 1, stop on Level 2 and 3 and escalate in its format. Do not restate the matrix; read it.

## Slicing and state

A slice is one vertical increment: data, logic, interface, tests. "Add an item, persist it, expose
it, test it" is a slice; "the backend" is not. It goes into `.agent/STATE.md`, with criteria, first.
...
Always this format, never an essay:
