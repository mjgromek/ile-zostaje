SLICE 2 — Zlecenie and dzieło, student status, on layout B
NOW: Tag held: the probe confirmed P1-J, the "Nie" chip overstates the relief by 37%% at 20 000 zł — builder is computing the real counterfactual   [00:26]

[x] Re-fetch and re-verify slice 1's eight cited sources
[x] Cite zlecenie ZUS, student exemption, KUP 20/50, the 50% annual cap, the relief contract list
[x] Capture the v0.1.0 umowa o pracę baseline from the tag
[x] Red: engine, citation and rendering tests as test:
[x] Green: per-contract costs in YearRates, zlecenie and dzieło engines as feat:
[x] Green: layout B, contract bar, Nie/Tak pills, notes, strings both languages as feat:
[x] Playwright: criteria 1, 2, 3, 4 in a real browser
[x] npm test and npm run build green, builder hands over
[x] Checker: nine criteria in a real browser, v0.1.0 regression, held-out — 1 P1, 3 P2
[x] Fix cycles 1 and 2: P1-A, P1-E, P2-B, P2-C closed; then P1-F closed
[x] Release check PASS, security gate PASS, architecture gate NO CHANGE
[x] Probe: P1-J confirmed — the Nie chip overstates the relief above ~10 319 zł
[ ] Fix P1-J with the counterfactual, then verify above and below the limit
[ ] Push main, annotated tag v0.2.0, README at v0.2.0, phase summary
